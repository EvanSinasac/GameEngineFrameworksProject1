...
//			        The purpose of this project is to show some clever design decisions to clean up and
//					and make our code more efficient.  Also, to show the implementation of patterns and techniques
//					discussed in class
...
//For Game Engine Project 2, need to remove these and make some kind of builder/factory
//Midterm
...
#include <physics/cCannonShotFactory.h>	//our new factory to make the cannon shots
...
int main(int argc, char** argv)
{
	...
	{
		cameraYaw = 180.f + (atan(cameraEye.z / cameraEye.x) * 180.f / glm::pi<float>());		//3.1459f
	}
...
static void key_callback(GLFWwindow* window, int key, int scancode, int action, int mods)
{
	...
	if (key == GLFW_KEY_1 && action == GLFW_PRESS)
	{
		...
		nPhysics::cCannonShot* shot = nPhysics::cCannonShotFactory::getFactory()->makeThisCannonShot("Bullet", 
			glm::vec3(cannonPosition.x, cannonPosition.y - 1, cannonPosition.z), 
			velocity, 
			&config,
			modelLocations[0]);
		world->AddParticle(shot);
		cannonShots.push_back(shot);
		g_vecMeshesToDraw.push_back(shot->getMesh());
	}
	if (key == GLFW_KEY_2 && action == GLFW_PRESS)
	{
		...
		nPhysics::cCannonShot* shot = nPhysics::cCannonShotFactory::getFactory()->makeThisCannonShot("Laser",
			glm::vec3(cannonPosition.x, cannonPosition.y - 1, cannonPosition.z),
			velocity,
			&config,
			modelLocations[0]);
	...
	if (key == GLFW_KEY_3 && action == GLFW_PRESS)
	{
		...
		nPhysics::cCannonShot* shot = nPhysics::cCannonShotFactory::getFactory()->makeThisCannonShot("Cannon Ball",
			glm::vec3(cannonPosition.x, cannonPosition.y - 1, cannonPosition.z),
			velocity,
			&config,
			modelLocations[0]);
	...
	if (key == GLFW_KEY_4 && action == GLFW_PRESS)
	{
		...
		nPhysics::cCannonShot* shot = nPhysics::cCannonShotFactory::getFactory()->makeThisCannonShot("Energy Ball",
			glm::vec3(cannonPosition.x, cannonPosition.y - 1, cannonPosition.z),
			velocity,
			&config,
			modelLocations[0]);
	...
	if (key == GLFW_KEY_5 && action == GLFW_PRESS)
	{
		...
		nPhysics::cCannonShot* shot = nPhysics::cCannonShotFactory::getFactory()->makeThisCannonShot("Firework",
			glm::vec3(cannonPosition.x, cannonPosition.y - 1, cannonPosition.z),
			velocity,
			&config,
			modelLocations[0]);
